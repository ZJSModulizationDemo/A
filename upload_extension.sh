...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' A_Extension.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" A_Extension.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' A_Extension.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" A_Extension.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' A_Extension.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" A_Extension.podspec
...
VersionLineNumber=`grep -nE 's.version.*=' A_Extension.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" A_Extension.podspec
...
pod repo push ${Repo} A_Extension.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
